...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route('admin.coupons.update', $coupon->id) }}">
            @csrf
            @method('PUT') <!-- This is for the PUT request to update the coupon -->

            <!-- Coupon Code -->
            <div class="mb-3">
                <label for="code" class="form-label">Coupon Code</label>
                <input type="text" id="code" name="code" class="form-control @error('code') is-invalid @enderror" placeholder="Enter coupon code" value="{{ old('code', $coupon->code) }}">
                @error('code')
            ...
            <div class="mb-3">
                ...
                <select id="limit_type" name="limit_type" class="form-control @error('limit_type') is-invalid @enderror">
                    ...
                    <option value="usage" {{ old('limit_type', $coupon->limit_type) == 'usage' ? 'selected' : '' }}>Usage</option>
                    <option value="time" {{ old('limit_type', $coupon->limit_type) == 'time' ? 'selected' : '' }}>Time</option>
                </select>
                @error('limit_type')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            ...
            <div class="mb-3" id="discount_amount_div">
                ...
                <input type="number" id="discount_amount" name="discount_amount" class="form-control @error('discount_amount') is-invalid @enderror" placeholder="Enter discount amount" value="{{ old('discount_amount', $coupon->discount_amount) }}" min="0" step="0.01">
            ...
            <div class="mb-3" id="discount_percentage_div">
                ...
                <input type="number" id="discount_percentage" name="discount_percentage" class="form-control @error('discount_percentage') is-invalid @enderror" placeholder="Enter discount percentage" value="{{ old('discount_percentage', $coupon->discount_percentage) }}" min="1" max="100" step="0.01">
            ...
            <div class="mb-3" id="max_usage_div">
                ...
                <input type="number" id="max_usage" name="max_usage" class="form-control @error('max_usage') is-invalid @enderror" placeholder="Enter max usage" value="{{ old('max_usage', $coupon->max_usage) }}" min="1">
                @error('max_usage')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3" id="expires_at_div">
                ...
                <input type="datetime-local" id="expires_at" name="expires_at" class="form-control @error('expires_at') is-invalid @enderror" value="{{ old('expires_at', $coupon->expires_at ? $coupon->expires_at->format('Y-m-d\TH:i') : '') }}">
                @error('expires_at')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="d-flex justify-content-end">
                <button type="submit" class="btn btn-primary">
                    <i class="bx bx-file"> </i>Save Changes
                </button>
